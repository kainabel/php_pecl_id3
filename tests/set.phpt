...
--FILE--
<?php
$path = dirname(__FILE__) . "/test.set.mp3";
...
var_dump(id3_remove_tag($path, ID3_V1_0));

printTagVersion($path);

var_dump(id3_set_tag($path, array("title" => "Re:Start", "artist" => "Re:\Legion", "comment" => "A nice track"), ID3_V1_1));

var_dump(id3_get_tag($path));

var_dump(id3_get_tag($path, ID3_V1_0));
